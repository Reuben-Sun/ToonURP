        ...
        Pass
        {
            ...
            float3 SampleSceneNormals(float2 uv, TEXTURE2D_X_FLOAT(_Texture), SAMPLER(sampler_Texture))
            {
                return UnpackNormalOctRectEncode(
                    SAMPLE_TEXTURE2D_X(_Texture, sampler_Texture, UnityStereoTransformScreenSpaceTex(uv)).xy) * float3(
                    1.0, 1.0, -1.0);
            }
            ...
            void EdgeDetectionFragment(Varyings input, out float4 outColor: SV_Target0)
            {
                ...
                float depthSampler = smoothstep(_EdgeThreshold.z * center.w, 0.0001f * center.w,
                                abs(center.w - neighborhood.w));
