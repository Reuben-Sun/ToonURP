Shader "ToonURP/ToonEye"
{
    Properties
    {
        ...
        [KWEnum(FeatureMode, Parallax, _PARALLAX)] _EnumFeatureMode ("Feature", float) = 0
        [Sub(FeatureMode)] [ShowIf(_EnumFeatureMode, Equal, 0)] _CustomFloat1 ("Parallax Scale", Range(0, 1)) = 1.0
        [Sub(FeatureMode)] [ShowIf(_EnumFeatureMode, Equal, 0)] _CustomFloat2 ("Mask Edge", Range(0, 1)) = 0.8
        [Sub(FeatureMode)] [ShowIf(_EnumFeatureMode, Equal, 0)] _CustomFloat3 ("Mask Edge Offset", Range(0, 1)) = 0.2
    ...
    SubShader
    {
        Pass
        {
            ...
            #define _ParallaxScale _CustomFloat1
            #define _MaskEdge _CustomFloat2
            #define _MaskEdgeOffset _CustomFloat3
            ...
            void PreProcessMaterial(inout InputData inputData, inout ToonSurfaceData surfaceData, float2 uv)
            {
                ...
                float3 viewDirWS = GetWorldSpaceNormalizeViewDir(inputData.normalWS);
                float3 viewDirOS = TransformWorldToObjectDir(viewDirWS);
                viewDirOS = normalize(viewDirOS);
                float2 parallaxOffset = viewDirOS.xy;
                parallaxOffset.y *= -1;
                float2 parallaxUV = uv + _ParallaxScale * parallaxOffset;
                //parallaxMask
                float2 centerVec = uv - float2(0.5, 0.5);
                half centerDist = dot(centerVec, centerVec);
                half parallaxMask = smoothstep(_MaskEdge, _MaskEdge + _MaskEdgeOffset, 1 - centerDist);
                float2 parallaxOffsetUV = lerp(uv, parallaxUV, parallaxMask);
                surfaceData.albedo = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, parallaxOffsetUV).rgb * _BaseColor.xyz;
